<?php 
	...
	class M_pindah_kelurahan extends CI_Model {

		public function tampil_data(){
			$this->db->join('data_penduduk', 'surat_pindah.nik=data_penduduk.nik');
			$this->db->order_by('tanggal_ajuan', 'DESC');
			return $this->db->get_where('surat_pindah', array('jenis_surat' => 'surat pindah kelurahan'));
		}

		// public function detail_data($id = NULL){
		// 	$this->db->join('data_penduduk', 'surat_pindah_kelurahan.nik=data_penduduk.nik');
		// 	$query = $this->db->get_where('surat_pindah_kelurahan', array('id' => $id))->row();
		// 	return $query;
		// }
		...
		// public function edit_id_nik_pindah($whereee, $table)
		// {
		// 	$this->db->select('id');
		// 	$query = $this->db->get_where($table, $whereee);
		// 	return $query;
		// }
		...
		// 	// $query = $this->db->query("SELECT nik_empat FROM surat_pindah WHERE id_surat_pindah = 'pindah-kelurahan-6322'");
		// 	$this->db->select('nik_empat');
		// 	$this->db->where($where);
		// 	$query = $this->db->get($table);
		// 	// $query = $this->db->get_where('surat_pindah', IN array('id_surat_pindah' => $where));
		// 	$nol = "0";
		// 	foreach ($query->result_array() as $row)
		// 	{
		// 	 if ($row['nik_empat'] !== $nol) {
