<?php 
	...
	class M_kematian extends CI_Model {
		...
		public function tampil_data(){
			$this->db->join('data_penduduk', 'surat_kematian.nik_jenazah=data_penduduk.nik');
			$this->db->order_by('status_2', 'DESC');
			return $this->db->get('surat_kematian');
		}

		// public function detail_data($id = NULL){
		// 	$this->db->join('data_penduduk', 'surat_kematian.nik_pelapor=data_penduduk.nik');
		// 	$query = $this->db->get_where('surat_kematian', array('id' => $id))->row();
		// 	return $query;
		// }
		...
		public function edit_data_saksi_satu($where, $table){
			$this->db->join('data_penduduk', 'surat_kematian.nik_saksi_satu=data_penduduk.nik');
			return $this->db->get_where($table, $where);
		}
		...
		public function edit_data_saksi_dua($where, $table){
			$this->db->join('data_penduduk', 'surat_kematian.nik_saksi_dua=data_penduduk.nik');
			return $this->db->get_where($table, $where);
		}
		...
		{
			$this->db->join('data_penduduk', 'surat_kematian.nik_jenazah=data_penduduk.nik');
			$data_kematian = $this->db->get_where('surat_kematian', array('id' => $id));
			return $data_kematian->result();
		}
